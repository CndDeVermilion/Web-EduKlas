...
// Log sesi untuk debugging
file_put_contents('edit_siswa_debug.log', "Sesi: " . print_r($_SESSION, true) . "\n", FILE_APPEND);

if (!isset($_GET['id'])) {
    $_SESSION['error'] = 'ID siswa tidak ditemukan di URL!';
    header('Location: data_siswa.php');
    exit;
}

$id = intval($_GET['id']);
file_put_contents('edit_siswa_debug.log', "ID dari GET: $id\n", FILE_APPEND);

$stmt = $conn->prepare("SELECT id_siswa, nama, username FROM siswa WHERE id_siswa = ?");
if (!$stmt) {
    $_SESSION['error'] = 'Gagal menyiapkan query: ' . $conn->error;
    file_put_contents('edit_siswa_debug.log', "Error prepare: " . $conn->error . "\n", FILE_APPEND);
    header('Location: data_siswa.php');
    exit;
}
...
if (!$stmt->execute()) {
    ...
    file_put_contents('edit_siswa_debug.log', "Error execute: " . $stmt->error . "\n", FILE_APPEND);
    $stmt->close();
    header('Location: data_siswa.php');
    exit;
}
...
if (!$data) {
    $_SESSION['error'] = 'Data siswa dengan ID ' . $id . ' tidak ditemukan di database!';
    file_put_contents('edit_siswa_debug.log', "Data tidak ditemukan untuk id_siswa: $id\n", FILE_APPEND);
    header('Location: data_siswa.php');
    exit;
}

file_put_contents('edit_siswa_debug.log', "Data ditemukan: " . print_r($data, true) . "\n", FILE_APPEND);
?>
...
<head>
    ...
    <title>Edit Siswa - EduKlas</title>
                    ...
                    <div class="col-md-12 grid-margin">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <?php endif; ?>
                                <form action="proses_edit_siswa.php" method="POST" class="forms-sample">
                                    ...
                                    <a href="data_siswa.php" class="btn btn-light">Batal</a>
                                </form>
                            </div>
                        </div>
                    </div>
